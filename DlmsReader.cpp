    ...
    else
    {
     // We have completed reading of one package, so clear and be ready for the next
      if (dataLength > 0 && position >= dataLength + 2)
      {
        ...
        {
          // Capture the length of the complete data package
...
/*
 * GetRawData
 * Return number of elements in Frame (elementCount)
 * dataBuffer will be copied from buffer[20] ... buffer[position-3]
 * 
 */
int DlmsReader::GetRawData(byte *dataBuffer, int *elements)
{
    int bytesWritten = 0;
    ...
    *elements = elementCount;
    if (dataLength > 0 && position == dataLength + 2)
    {
        for (int i = 20; i < dataLength - 1; i++)
        {
          dataBuffer[i - 20] = buffer[i];
          bytesWritten++;
        }
        return bytesWritten;
    }
